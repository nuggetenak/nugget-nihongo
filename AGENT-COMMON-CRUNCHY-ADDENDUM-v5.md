...
> Versi: v5 (TASK-GOV-017-C, Maret 2026)
> Status: APPROVED
...
Tipe: main project ZIP + PATCH ZIP (CP(2))
  ...
  Catatan packaging CP(2): DISPATCH-CLEARANCE.md dari Task Brief asli agent wajib
  di-include ke archive. Crunchy mengambilnya dari patch ZIP yang di-submit agent
  (agent wajib menyertakan DISPATCH-CLEARANCE.md dari Task Brief ZIP mereka di dalam
  PATCH ZIP) atau dari Task Brief ZIP asli jika masih tersedia di environment.
  ...
  7. ZIP → FOR-INTEGRATOR_[taskID].zip
     └── for-integrator-crunchy/
         ...
         └── archive/                          ← WAJIB ADA (TASK-INTEL-P0A)
             ...
             │   ├── patch-notes/
             │   ├── session-proofs/
             │   └── dispatch-clearances/      ← WAJIB ADA (GOV-017-C)
